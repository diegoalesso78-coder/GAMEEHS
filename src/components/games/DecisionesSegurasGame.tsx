  ...
  if (gameState === 'START') {
    return (
      <div className="h-screen flex items-center justify-center p-4 obsidian-table relative overflow-hidden">
        ...
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="glass-panel-heavy w-full max-w-2xl p-12 rounded-2xl border border-secondary/30 text-center hard-shadow relative z-10">
          <ShieldCheck className="mx-auto text-secondary mb-8" size={80} />
          <h2 className="text-5xl font-headline font-black mb-4 tracking-tighter uppercase">DECISIONES <span className="text-secondary">SEGURAS</span></h2>
          <p className="text-sm opacity-70 font-body mb-10 max-w-md mx-auto">Enfrentá situaciones reales de planta y demostrá que tu prioridad es la vida. Cada decisión cuenta.</p>
          <div className="flex flex-col gap-4">
            <button onClick={() => setGameState('PLAYING')} className="btn-industrial-orange w-full py-5 text-black font-headline font-black uppercase tracking-widest text-sm">INICIAR ENTRENAMIENTO</button>
  ...
  if (gameState === 'SUMMARY') {
    return (
      <div className="h-screen flex items-center justify-center p-4 obsidian-table relative overflow-hidden">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-panel-heavy w-full max-w-3xl p-10 rounded-2xl border-2 border-secondary hard-shadow relative z-10 flex flex-col max-h-[90vh]">
          <div className="text-center mb-8">
            <Trophy className="mx-auto text-secondary mb-4" size={60} />
            <h2 className="text-4xl font-headline font-black mb-1 tracking-tighter uppercase">ENTRENAMIENTO FINALIZADO</h2>
            <p className="text-xl font-headline font-black text-secondary uppercase tracking-widest">Puntaje: {score}/100</p>
          </div>
    ...
    <div className="h-screen flex flex-col p-4 md:p-8 obsidian-table relative overflow-hidden">
      ...
      <header className="flex justify-between items-center mb-8 relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-headline font-black tracking-tighter uppercase leading-none">DECISIONES <span className="text-secondary">SEGURAS</span></h1>
            <p className="text-[10px] font-headline uppercase tracking-widest opacity-50">Escenario {currentIndex + 1} de {scenarios.length}</p>
          </div>
        </div>
      ...
      <main className="flex-1 flex items-center justify-center relative z-10">
        <AnimatePresence mode="wait">
          {gameState === 'PLAYING' ? (
            <motion.div key="scenario" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="w-full max-w-4xl grid md:grid-cols-2 gap-8">
              <div className="glass-panel-heavy p-8 rounded-2xl border border-white/10 flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-6">
                  <AlertTriangle className="text-secondary" size={32} />
                  <h3 className="text-2xl font-headline font-black uppercase tracking-tight leading-none">{current.escenario}</h3>
                </div>
                <p className="text-lg text-on-surface-variant font-body leading-relaxed mb-8">{current.descripcion}</p>
              ...
              <div className="flex flex-col gap-4">
                {['A', 'B', 'C'].map((opt) => (
                  <button
                    ...
                    className="group relative p-6 bg-white/5 border border-white/10 rounded-xl hover:border-secondary hover:bg-secondary/10 transition-all text-left overflow-hidden"
                  >
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 text-4xl font-headline font-black opacity-5 group-hover:opacity-10 transition-opacity">{opt}</div>
                    <p className="text-sm font-headline font-black uppercase tracking-widest pr-12">{current[`opcion_${opt.toLowerCase()}`]}</p>
                  </button>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div key="feedback" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="w-full max-w-2xl text-center">
              ...
              <h3 className={`text-4xl font-headline font-black mb-4 uppercase tracking-tighter ${selectedOption === current.correcta ? 'text-emerald-400' : 'text-rose-500'}`}>
                {selectedOption === current.correcta ? '¡DECISIÓN CORRECTA!' : 'DECISIÓN RIESGOSA'}
              </h3>
              
              <div className="glass-panel-heavy p-8 rounded-2xl border border-white/10 mb-10">
                <p className="text-xl font-body leading-relaxed mb-6">
                ...
                <p className="text-xs font-headline font-black uppercase tracking-widest text-secondary">Recuerda: {current.principio}</p>
              </div>

              <button onClick={nextScenario} className="btn-industrial-orange px-12 py-5 text-black font-headline font-black uppercase tracking-widest text-sm">
